...
int usersmodelview::rowCount(const QModelIndex &parent) const
{
    return v.size();
}

//METODO RESTITUISCE IL NUMERO DI COLONNE DELLA TABELLA
int usersmodelview::columnCount(const QModelIndex &parent) const
{
    return 4;
}

//METODO SERVE PER CARICARE DATI NEL MODELLO(TABELLA IN QUESTO CASO)
QVariant usersmodelview::data(const QModelIndex &index, int role) const
{
...
QVariant usersmodelview::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (role == Qt::DisplayRole)
    {
...
bool usersmodelview::addUser(QString type, QString n, QString sn, QString un, QString pass, QDate bd)
{
...
bool usersmodelview::removeUser(QString un)
{
    bool cond = false;
    int row = 0;
...
user *usersmodelview::find(QString un) const
{
...
void usersmodelview::loadUsers(QFile &f)
{
...
void usersmodelview::loadSingleUser(QXmlStreamReader &file)
{
...
bool usersmodelview::saveData(QFile &file) const
{
